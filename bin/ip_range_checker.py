...
import modalert_ip_range_checker_helper

class AlertActionWorkerip_range_checker(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerip_range_checker, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_ip_range_checker_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerip_range_checker("TA-securitytrails", "ip_range_checker").run(sys.argv)
    sys.exit(exitcode)
